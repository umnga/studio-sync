/**
 * Piano Synthesizer Hook
 * Generates MIDI-style piano sounds using Web Audio API
 */
...
// Frequencies for each MIDI note (C0 to C8)
...
export const usePianoSynthesizer = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      audioContextRef.current.oscillators.clear();
    };
  }, []);

  // Play a single note
  const playNote = (note: string, duration: number = 0.5, velocity: number = 0.8) => {
    const { context, gainNode, oscillators } = audioContextRef.current;
    ...
    // Connect: oscillator -> noteGain -> mainGain -> destination
  ...
  const playChord = (notes: string[], duration: number = 1, velocity: number = 0.7) => {
    notes.forEach((note) => {
  ...
  const playArpeggio = (notes: string[], tempo: number = 120, velocity: number = 0.6) => {
    const beatDuration = (60 / tempo) * 1000; // Convert BPM to milliseconds
    notes.forEach((note, idx) => {
      setTimeout(() => {
        playNote(note, 0.3, velocity);
      }, idx * (beatDuration / 2));
    });
  };
  ...
  const setVolume = (value: number) => {
    if (audioContextRef.current.gainNode) {
      audioContextRef.current.gainNode.gain.value = Math.max(0, Math.min(1, value));
    }
  };
  ...
  return {
    ...
    playArpeggio,
    setVolume,
    audioContext: audioContextRef.current.context,
  };
};

export default usePianoSynthesizer;
